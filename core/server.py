...
class DCFCLServer:
    ...
    def _init_clients(self):
        ...
        # 延迟导入避免循环依赖（FL_model → core → core.server → FL_model）
        ...
        for i, name in enumerate(client_names):
            ...
            # 根据 config.algorithm 创建对应算法的客户端实例
    ...
    def _init_algorithm(self):
        ...
        if self.config.algorithm in ['DCFCL', 'CollEdge', 'ClusterFL']:
    ...
    def _local_training(self, global_round: int, task: int) -> Dict:
        ...
        for client in self.clients:
            ...
            if self.config.algorithm in ['DCFCL', 'CollEdge']:
    ...
    def _server_aggregation(self, global_round: int, task: int, train_results: Dict):
        ...
        elif algorithm in ['DCFCL', 'CollEdge', 'ClusterFL']:
