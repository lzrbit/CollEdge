...
# -*- coding: utf-8 -*-
"""
core/client.py — 向后兼容层

所有联邦学习算法的客户端实现已迁移至顶层 FL_model/ 目录。
每种算法对应一个独立文件，继承自 FL_model.base_client.BaseClient。

本文件保留 Client / create_client 的导入接口，供现有代码无缝调用。
"""
...
class Client:
    ...
    def __init__(self, client_id: int, config, model, train_data, test_data, 
        ...
        # Replay buffer for CollEdge (DER/DER++)
        self.replay_buffer = None
        if config.algorithm == 'CollEdge':
    ...
    def train(self, glob_iter: int, task: int, **kwargs) -> Dict[str, Any]:
        ...
        elif algorithm == 'CollEdge':
            return self._train_colledge(kwargs.get('proto_queue'))
        else:
    ...
    def _train_colledge(self, proto_queue=None) -> Dict[str, Any]:
        """
        CollEdge: DCFCL + Dark Experience Replay (DER++).
