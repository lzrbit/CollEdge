#!/usr/bin/env bash
# ==============================================================
# 04_ablation_cifar100.sh
# Purpose : CollEdge 全模块消融实验（CIFAR-100）
#
# 消融矩阵（7 组实验）：
# ┌─────────────────────────────┬───────┬──────────┬──────┐
# │ 实验                        │ DER++ │ 有向协作 │ Mask │
...
#
# 说明：
#   - 以 configs/dcfcl_cifar100.yaml 为基础配置
#   - 10 个客户端 / 2 组 = 每组 5 个（Mask 为 group 类型）
...
#
# Results : results/paper_experiments/ablation/CIFAR100/
# Usage   : bash scripts/paper_experiments/04_ablation_cifar100.sh
# ==============================================================

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO"

ABL_BASE="results/paper_experiments/ablation/CIFAR100"
CFG="configs/dcfcl_cifar100.yaml"

_log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"; }

_log "=== START 04_ablation_cifar100.sh ==="
...
_log "=== END 04_ablation_cifar100.sh ==="
