...
Part 2: Ablation study for CollEdge.
...
ALL_ALGORITHMS = [
    'DCFCL', 'CollEdge', 'FedAvg', 'FedProx', 'FedLwF',
    'Local', 'SCAFFOLD', 'PerAvg', 'pFedMe', 'ClusterFL', 'L2C',
]

# ============================================================
# Ablation configurations for CollEdge on EMNIST-Letters
# ============================================================
ABLATION_CONFIGS = {
    # baseline (full CollEdge)
...
def write_markdown(all_results, ablation_results, md_path="EXPERIMENT_RESULTS.md"):
    ...
    # ---- Part 2: Ablation ----
    if ablation_results:
        lines.append("\n## CollEdge Ablation Study (EMNIST-Letters)\n")
...
def main():
    ...
    # ================================================================
    # Part 2: Ablation study for CollEdge on EMNIST-Letters
    # ================================================================
    print(f"\n{'#'*60}")
    print(f"# Part 2: CollEdge Ablation Study — {len(ABLATION_CONFIGS)} configs")
    print(f"{'#'*60}")
    ablation_results = {}
    for tag, overrides in ABLATION_CONFIGS.items():
        ...
        cmd = build_cmd(EMNIST_BASE, "CollEdge", **overrides)
        r = run_one(f"CollEdge ({tag})", cmd)
